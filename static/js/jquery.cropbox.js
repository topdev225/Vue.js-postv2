    ...
    Crop.prototype = {
      ...
      fit: function () {
        ...
        if (widthRatio > 1.0 | heightRatio > 1.0) {
          ...
          ratio = 100 * (widthRatio >= heightRatio) ? widthRatio : heightRatio;
        }
        this.minPercent = ratio * 100.0;
      },

      setCrop: function (result) {
        var ratio = Math.max(this.options.width/result.cropW, this.options.height/result.cropH);
        this.img_width = Math.ceil(this.width*ratio);
        this.img_height = Math.ceil(this.height*ratio);
        this.img_left = -Math.floor(result.cropX*ratio);
        this.img_top = -Math.floor(result.cropY*ratio);
        this.$image.css({ width: this.img_width, left: this.img_left, top: this.img_top });
        this.percent = 100 * ratio;
        this.update();
      },

      zoom: function(percent) {
        var old_percent = this.percent;
        var maxPercent = this.minPercent * this.options.maxZoom;
      ...
      zoomIn: function() {
        var maxPercent = this.minPercent * this.options.maxZoom;
        var zoomIntervals = (maxPercent - this.minPercent) / this.options.zoom;
        this.zoom(this.percent + zoomIntervals);
      },
      zoomOut: function() {
        var maxPercent = this.minPercent * this.options.maxZoom;
        var zoomIntervals = (maxPercent - this.minPercent) / this.options.zoom;
      ...
      update: function() {
        var ratio = this.percent / 100.0;
        this.result = {
          cropX: -Math.ceil(this.img_left / ratio),
          cropY: -Math.ceil(this.img_top / ratio),
          cropW: Math.floor(this.options.width / ratio),
          cropH: Math.floor(this.options.height / ratio),
          stretch: this.minPercent > 100
        };
